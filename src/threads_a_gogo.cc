...
struct eventsQueue {
   eventsQueue() : first(nullptr),last(nullptr)
      {}
...
typeThread::typeThread():
   ...
   id(threadsCtr++),
   //      thread
   IDLE(0), ended(0), sigkill(0), destroyed(0), hasDestroyCallback(0),
   hasIdleEventsListener(0), threadMagicCookie(kThreadMagicCookie),
   threadEventsQueue(nuQueue()),
   processEventsQueue(nuQueue()),
   isolate(nullptr)
   {
   memset(&async_watcher, 0, sizeof(uv_async_t));
   }
...
//      v8::Local<v8::Value> ptr = receiver->GetHiddenValue(v8::String::NewFromUtf8(v8::Isolate::GetCurrent(), "ptr"));
      v8::MaybeLocal<v8::Value> ptr = Nan::GetPrivate(receiver, Nan::New("ptr").ToLocalChecked());
      if (ptr.IsEmpty())
         return nullptr;
      thread = (typeThread*)((uintptr_t)ptr.ToLocalChecked()->ToNumber()->Value());
#else
...
static void wakeUpThread(typeThread* thread, int sigkill) {
   ...
   std::unique_lock<std::mutex> lock(thread->idle_mutex);
...
//Runs at thread start up.
//Creates v8 isolate (i.e. a GC heap) then runs eventloop until thread ends
static void* threadBootProc(void* arg) {
   ...
   if (useLocker) {
      ...
      //Ensure no other threads can use 
      v8::Locker locker(thread->isolate);

      //Enter the locked isolate with this thread
      v8::Isolate::Scope isolateScope(thread->isolate);
...
static void eventLoop(typeThread* thread) {
   ...
   while (1) {
         ...
         if (event->eventType == kEventTypeLoad) {
            ...
            if (event->load.hasCallback) {
               if (!event->load.error)
                  ...
                  event->eval.resultado = _strdup("fopen(path) error");
      ...
      if (!event && !thread->sigkill) {
         ...
         thread->IDLE = 1;
         if (thread->hasIdleEventsListener) 
            notifyIdle(thread);
...
//Because most of the code is identical in processEmit and threadEmit
...
//La que emite los events de node hacia las threads -issues the node events to threads
...
//The one that issues the events of the threads towards node, is executed in the threads
...
//   char* errstr;
   try
      {
      thread->thread.swap(std::thread(threadBootProc, thread));
      }
   catch (std::exception &ex)
      {
      printf("THREAD %ld PTHREAD_CREATE() exception: %s : %s\n", thread->id, typeid(ex).name(), ex.what());
...
//   nodeObject->SetHiddenValue(v8::String::NewFromUtf8(iso, "ptr"), v8::Number::New(iso, (double)((uintptr_t)thread)));
   Nan::SetPrivate(nodeObject,v8::String::NewFromUtf8(iso, "ptr"), v8::Number::New(iso, (double)((uintptr_t)thread)));
...
void Init(v8::Handle<v8::Object> target) {
   ...
   TAGG_DEBUG && printf("TAGG useLocker is %d\n", useLocker);
